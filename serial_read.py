...
def readMessage():
  global ser
  global badmessageheaderCount, sinceLastGoodMessage
  try:
    x = 0
    while x != 5:
      t = ser.read(1)
      sinceLastGoodMessage += 1
      print_xy(3,0,"SinceLast good msg: %d \tBad message header found: %d  "%(sinceLastGoodMessage,badmessageheaderCount) )
      ...
      if len(MessageHeader) == 6:
        ...
        if msgType == 2 : # GPS Message
          ...
          if len(Message) == 36:
            ...
            print_xy(15,24,bcolors.OKGREEN+"GPS"+bcolors.ENDC)
